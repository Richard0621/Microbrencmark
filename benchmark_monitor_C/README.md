...
### Error: "Google Benchmark no encontrado"
...
### Warning: "RAPL no disponible"

- Normal en CPUs no-Intel o VMs
- El benchmark funciona, pero `energy_*` será 0
- Ejecuta con `sudo` para acceder a RAPL

### Error de compilación: "undefined reference to benchmark"

Verifica que Google Benchmark esté compilado:

```bash
ls ../build/lib/libbenchmark.a
# Si no existe:
...
| Aspecto | C++ (este proyecto) | Python (benchmark_monitor) |
|---------|---------------------|----------------------------|
| **Velocidad** | 🚀 10-100x más rápido | ✓ Más lento pero flexible |
| **Precisión** | ⭐ Muy alta | ✓ Buena |
| **Compilación** | ⚠️ Requiere compilar | ✅ Listo para ejecutar |
| **Métricas** | ✅ Completas | ✅ Completas |
| **Uso de memoria** | ✅ Eficiente | ⚠️ Mayor overhead |
| **Modificación** | ⚠️ Requiere recompilar | ✅ Editar y ejecutar |

**Recomendación**: 
- Usa **C++** para benchmarks de producción y máximo rendimiento
- Usa **Python** para prototipado rápido y análisis exploratorio
...
```bash
# Ejecutar con diferentes governors
...
```bash
# Antes de optimizar
./build.sh && sudo ./run_benchmark_with_perf.sh
cp results_cpp.csv results_before.csv

# [Hacer optimizaciones en el código]

# Después de optimizar
./build.sh && sudo ./run_benchmark_with_perf.sh
cp results_cpp.csv results_after.csv

# Analizar mejoras
```

### 3. Comparar Niveles de Optimización del Compilador

Edita `CMakeLists.txt` y prueba con `-O0`, `-O1`, `-O2`, `-O3`:
...
Este proyecto es código educativo y puede modificarse libremente.

## 🤝 Contribuciones

Mejoras sugeridas:
- [ ] Añadir más microbenchmarks (FFT, sorting, etc.)
- [ ] Soporte para AMD energy counters
- [ ] Gráficas automáticas con Python/matplotlib
- [ ] Integración con Google Benchmark JSON output
...
# 2. Test rápido (sin sudo)
./benchmark_monitor --benchmark_min_time=0.1

# 3. Test completo (con sudo)
sudo ./run_benchmark_with_perf.sh

# 4. Verificar salida
[ -f results_cpp.csv ] && echo "✅ CSV generado" || echo "❌ Error"
```

---

**¡Todo listo!** Ejecuta `./build.sh` para compilar y luego `sudo ./run_benchmark_with_perf.sh` para comenzar. 🚀
